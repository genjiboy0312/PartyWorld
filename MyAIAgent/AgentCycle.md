...
- 사용자 응답은 한국어 존댓말을 기본으로 유지합니다.
- 주석 추가는 사용자가 명시적으로 요청한 경우에만 수행합니다.
- 런타임 `Find*` 의존 대신 인스펙터 참조 + `Debug.LogWarning` 기반 누락 노출 정책을 유지합니다.
- 신규 에디터 유틸 메뉴는 `GenJiTools` 상위 메뉴 아래로 정리합니다.
