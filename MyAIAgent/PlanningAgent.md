...
`Scene_Title&Login → (선택) Scene_CharacterCreation → Scene_WaitingRoom → Scene_Lobby → Scene_Loading → Scene_MapXX`
...
### 2.5 진단 기반 즉시 보정 포인트(2026-03-25)

- Build Settings에는 `Scene_CharacterCreation`이 포함되어 있으므로, 실제 런타임 플로우에서 진입 조건(신규 유저/기존 유저)을 명시해야 함
...
- 기본 플로우(`Title&Login → WaitingRoom → Lobby → Loading → Map → Result → Lobby`)가 3판 연속으로 끊김 없이 동작
- 8인 기준에서 Ready/취소/재시작(카운트다운) 시나리오가 모두 정상 동작
- 승자 판정(1인 생존) 후 결과 표시 및 로비 복귀가 전원 동기화 상태로 완료
- 주요 실패 케이스(마스터 이탈, 로딩 지연, 중도 퇴장)에서 정의된 폴백으로 복구
...
2. 로그인 성공 시:
    ...
    - 캐릭터 생성이 필요한 계정이면 `Scene_CharacterCreation` 경유 후 `Scene_WaitingRoom`로 이동
    - 캐릭터 생성이 완료된 계정이면 `Scene_WaitingRoom`로 바로 이동
...
1. 스폰:
    ...
    - 플레이어 프리팹 생성 책임은 `NetworkAuthorityManager`로 단일화(콜백 트리거는 허용하되 소유권은 단일 매니저)
2. 진행:
   - 라운드 타이머(선택)
...
| Phase | 진입 이벤트 | 권위 주체(Owner) | 완료 조건 | Timeout | 실패 시 폴백 |
|:--|:--|:--|:--|:--|:--|
| `Auth` | 로그인 성공 | 클라이언트 + Auth | 계정 인증 완료 | 15s | 로그인 UI로 복귀, 에러 메시지 표시 |
| `WaitingRoom` | `Scene_WaitingRoom` 진입 | `NetworkAuthorityManager` | Photon 접속 + QuickPlay 가능 상태 | 15s | 재접속 버튼 활성화, 3회 재시도 |
| `Matchmaking` | `StartQuickPlay()` 호출 | 마스터 후보/서버 응답 | 룸 입장 성공 | 20s | `JoinRandomRoom` 실패 시 `CreateRoom` |
| `LobbyReady` | `Scene_Lobby` 진입 | 룸 마스터 | 전원 `ready=true` | 60s | 미완료 인원 안내, 타이머 리셋 |
| `Countdown` | 전원 Ready 충족 | 룸 마스터 | 카운트다운 0 도달 | 5s | Ready 해제/퇴장 시 즉시 취소 |
| `LoadingGate` | `Scene_Loading` 진입 | 룸 마스터 | 전원 로딩 도착 확인 | 30s | 미도착 인원 표시 후 재동기화 시도 |
| `Round` | 맵(`selectedMap`) 로드 완료 | 룸 마스터 + 각 클라 | 생존자 1명 판정 | 라운드 정책값 | 타임아웃 시 상위 N명 점수 판정(정책 확정 필요) |
| `Result` | 승자 확정 | 룸 마스터 | 결과 UI 표시 + 다음 씬 결정 | 15s | 결과 생략 후 `Scene_Lobby` 강제 복귀 |
...
3. Map01에서 탈락 판정/승자 결정/Result 표시/로비 복귀까지 완결
4. 플레이어 스폰 책임을 `NetworkAuthorityManager` 기준으로 문서/코드 모두 고정
5. `Scene_Map*` 다중 맵에서도 `GameState.Playing` 동기화가 누락되지 않도록 상태 매핑 보강

### P0.5 (리팩토링 후속 조치)

1.  **레거시 스크립트 및 오브젝트 완전 제거:** 식별된 레거시 스크립트(`PhotonManager` 등)를 프로젝트에서 삭제하고, 자동 생성 방식으로 인해 불필요해진 전역 매니저 게임 오브젝트를 모든 씬(`Title`, `Lobby` 등)에서 제거. (사용자 직접 수행 필요)
2.  **데이터 영속성 구현:** `DataManager`에 `// TODO:`로 명시된 Firebase 연동 기능을 구현하여 유저 데이터(재화, 아이템 등)를 실제로 저장하고 로드하는 기능 개발.
3.  **UI 시스템 고도화:** 현재 씬별로 나뉜 UI 매니저(`UIManager`, `ChatManager` 등)들을 총괄할 수 있는 중앙 UI 관리 시스템(예: 팝업, 씬 전환 효과, 공통 컴포넌트 관리) 도입 검토.
4.  **프리팹(Prefab) 정리:** 레거시 스크립트(`PlayerCheckingManager` 등)가 연결된 프리팹이 있는지 확인하고, 있다면 해당 컴포넌트 제거.
...
- 해당 내용은 별도 에이전트 문서로 분리해서 관리합니다: `RiskAgent.md`
