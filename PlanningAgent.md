...
`Scene_Title&Login → Scene_Lobby → Scene_Loading → Scene_Map01`
...
- Photon 연결/룸 입장 로직이 여러 곳에 분산되어 **상태 꼬임 위험**이 큼
  - `PhotonManager`: `ConnectUsingSettings()` + 랜덤 룸/생성, 스폰
  - `ChatManager`: 별도 `ConnectUsingSettings()` + `Room1(MaxPlayers=4)` 입장/생성
  - `PlayerCheckingManager`: `ConnectUsingSettings()` 시도 + 로딩/레디 카운트 로직
- `PlayerCheckingManager.cs`에 `using UnityEditor;`가 포함되어 있어 **빌드 시 문제 소지**가 있음(런타임 코드에 Editor 의존성 혼입)
...
### 4.2 Lobby (`Scene_Lobby`)

로비 목표: 파티/매칭 + 룸 참가 + 준비(Ready) + 게임 시작 합의.

권장 동작:
1. “단일 네트워크 매니저”가 Photon 연결 상태를 확보
2. 룸 입장(랜덤 매칭 또는 방 생성)
3. 유저 리스트 / 채팅은 **룸 상태를 구독만** 하고, 룸 생성/입장/연결을 직접 수행하지 않음
4. Ready 버튼:
   - 각 플레이어가 `Player.CustomProperties`에 `ready=true` 설정
   - 마스터가 모두 준비 완료면 로딩 시작

### 4.3 Loading (`Scene_Loading`)
...
2. 마스터가 Ready 조건 만족 시:
   ...
   - `PhotonNetwork.LoadLevel("Scene_Map01")` 호출

### 4.4 Map01 (`Scene_Map01`) — 라운드(1인 생존)
...
3. 종료:
   - 생존자 수가 1명 → 결과(승자 표시) → 로비 복귀/다음 라운드
4. 관전:
...
- 연결/로비/룸 입장/씬 전환은 **오직 한 매니저(예: `PhotonManager` 역할)** 에서만 수행
...
- [ ] 런타임 코드에서 `UnityEditor` 의존성 제거(빌드 안정성)
- [ ] `ChatManager`의 `MaxPlayers=4`, `PhotonManager`의 `MaxPlayers=10` 등 정책 불일치 정리
- [ ] `ConnectUsingSettings()`가 여러 군데서 호출되지 않도록 정리
- [ ] 씬 로드는 멀티 상황에서 `PhotonNetwork.LoadLevel`로 통일
