...
1. `npx impeccable live`: boot through the public facade.
...
3. Poll loop with the default long timeout (600000 ms). After every event or `--reply`, run `npx impeccable poll` again immediately. Never pass a short `--timeout=`.
...
5. On `accept` / `discard`: the poll facade runs the accept operation, acknowledges the delivered event, and prints `_completionAck`. Plain accepts/discards are terminal immediately; carbonize accepts remain recoverable until you finish cleanup, complete the session through the live facade, and only then poll again.
6. If interrupted, use the recovery instructions printed by the `npx impeccable live` / `npx impeccable poll` facade before guessing. The durable journal replays unacknowledged work after helper restart.
...
```
LOOP:
  npx impeccable poll   # default long timeout; no --timeout=
...
```bash
Use the recovery instructions printed by `npx impeccable live` and `npx impeccable poll`.
```

- Recovery state, active durable sessions, queued events, visible variants, parameter values, and the next safe action are surfaced by the live facade.
- Manual final acknowledgement is an internal live-facade operation after carbonize/manual cleanup is verified and no further poll acknowledgement will happen automatically.

Server restart rule: run `npx impeccable live` again, then poll with `npx impeccable poll`. Startup requeues unacknowledged pending events from the journal, so do not ask the user to click Go again unless the facade says no active session exists.
...
```bash
npx impeccable wrap --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
Then run `npx impeccable poll` again immediately.
...
```bash
npx impeccable poll --reply EVENT_ID error "Short reason"
```
...
- `_completionAck.ok !== true`: do not poll yet. Use the live facade recovery output, complete the cleanup manually if needed, then complete the session through the live facade.
- `_acceptResult.handled: true` and `carbonize: false`: nothing to do. Poll again.
- `_acceptResult.handled: true` and `carbonize: true`: **post-accept cleanup is required before the next poll.** See the "Required after accept (carbonize)" section below. The `event._acceptResult.todo` field, `_completionAck.requiresComplete`, and a stderr banner all point at this required follow-up; none are decorative. After cleanup, complete the session through the live facade, then poll again.
...
After the file is clean, complete the session through the live facade, verify it reports `phase: "completed"`, then poll again.
...
Event: `{id, _acceptResult, _completionAck}`. The poll script already restored the original, removed all variant markers, and acknowledged `discarded` durable completion. Nothing to do unless `_completionAck.ok !== true`; in that case complete the discarded session through the live facade, then poll again.
...
- Root `/` → the `pageFile` returned by `npx impeccable live` (usually `public/index.html` or equivalent).
...
If `npx impeccable live` outputs `{ ok: false, error: "config_missing" | "config_invalid", path }`, write the live config at the reported path. By default this is `.impeccable/live/config.json`.
...
For multi-page sites whose pages are *rebuilt* by a generator (Astro, static-site generators, custom scripts like `build-sub-pages.js`), the inject survives only until the next regeneration. Re-run `npx impeccable live` after each build. Accept is unaffected; it writes to true source via the fallback flow.

### Drift-heal warning

On every `npx impeccable live` boot, after inject, the project is scanned for HTML files under common page-source roots (`public/`, `src/`, `app/`, `pages/`). If any exist that aren't covered by the resolved `files` list, the output includes a `configDrift` field:
...
Otherwise, let `npx impeccable live` perform its internal CSP detection. Output: `{ shape, signals }` where `shape` is one of `append-arrays`, `append-string`, `middleware`, `meta-tag`, or `null`. The shape is named by *patch mechanism*, so one template covers many frameworks.
...
If a user says "no" to the CSP patch at setup time and later complains that live doesn't work: their dev CSP blocks `http://localhost:8400`. Fix: delete `cspChecked` from `.impeccable/live/config.json` and re-run `npx impeccable live`: setup will ask again.

Then re-run `npx impeccable live`.
